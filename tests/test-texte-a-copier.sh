#!/usr/bin/env bash
# CONTROLE MINIMAL DE garde-texte-a-copier
#
...
# tes fautes a toi.
set -u
G="$(dirname "$0")/../gates/garde-texte-a-copier.py"
VERT=0; ROUGE=0
ok()  { printf 'VERT   %s\n' "$1"; VERT=$((VERT+1)); }
nok() { printf 'ROUGE  %s\n' "$1"; ROUGE=$((ROUGE+1)); }

python3 -c "import ast,sys;ast.parse(open('$G',encoding='utf-8').read())" 2>/dev/null \
  && ok "garde-texte-a-copier se lit sans faute" || nok "garde-texte-a-copier a une faute d ecriture"
